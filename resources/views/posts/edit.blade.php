...
@section('content')
    ...
    <div class="content">
        ...
        <form action="{{ url('/posts/actualizar/'.$post->id) }}" method="POST" endtype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="container-fluid">
                                ...
                                <div class="form-group">
                                    ...
                                    <select type="text" name="categoria_id" id="categoria_id"  class="form-control">
                                        ...
                                        @foreach ($categorias as $categ )
                                            <option value="{{ $categ->id }}"@if($categ->id== $post->categoria_id) selected @endif> {{ $categ->nombre}} </option>
                                        @endforeach
                                    </select>
                                    @error('categoria_id')
                                        <small class="text-danger">{{ $message }}</small>
                                    ...
                                    <select name="tags" id="tags"  class="form-control" multiple><!--multiple-->
                                        ...
                                        @foreach ($tags as $item )
                                            <option value="{{ $item->nombre }}"
                                                {{-- post->tags es un array, y a ese array le preguntamos si existe, si existe que lo seleccione --}}
                                                @if( in_array($item->nombre, json_decode($post->tags)))
                                                selected
                        ...
                        <div class="card m-1">
                            <div class="card-body">
                                <div class="form-grpup">
                                    ...
                                    <input type="datetime-local" name="fecha_publicacion" class="form-control" value="{{ $post->fecha_publicacion}}">
